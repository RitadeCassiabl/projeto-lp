        ...
        with open("database/medicamentos.txt", "r", encoding="utf-8") as arquivo:
            ...
            while i < len(linhas):
                ...
                if linha_atual.startswith("nome:"):
                    ...
                    if i + 3 < len(linhas) and linhas[i + 3].strip().startswith(
                        "preco:"
                    ):
                        preco_str = linhas[i + 3].split(":", 1)[1].strip()

                        if i + 4 < len(linhas) and linhas[i + 4].strip().startswith(
                            "quantidade:"
                        ):
                            ...
                            try:
                                ...
                                print(
                                    f"Aviso: Dado inválido para '{nome}'. Preço: '{preco_str}', Qtd: '{qtd_str}'. Ignorando."
                                )
    ...
    try:
        ...
        while i < len(linhas):
            ...
            if linha_atual.strip().startswith("nome:"):
                ...
                if (
                    nome in itens_vendidos
                    and i + 4 < len(linhas)
                    and linhas[i + 4].strip().startswith("quantidade:")
                ):
                    ...
                    try:
                        ...
                        if nova_qtd < 0:
                            nova_qtd = 0
                            print(
                                f"Aviso: Estoque de '{nome.capitalize()}' não pode ser negativo. Definido para 0."
                            )
                        ...
                        print(
                            f"Erro ao processar quantidade para '{nome.capitalize()}'. Mantendo original."
                        )
...
class MenuCaixa:
    ...
    def processar_nova_venda(self):
        ...
        pagamento_input = (
            input("\nDigite o método de pagamento (dinheiro, pix, debito, credito): ")
            .strip()
            .upper()
        )
        ...
        itens_input = input(
            "Itens da venda (ex: nome do medicamento:quantidade, nome2:quantidade2, ...): "
        ).strip()
        ...
        for nome_item, quantidade_pedida in itens_venda.items():
            if nome_item in medicamentos_data:
                ...
                if quantidade_pedida > item_info["quantidade"]:
                    print(
                        f"Erro: Estoque insuficiente para '{nome_item.capitalize()}'. Pedido: {quantidade_pedida}, Estoque: {item_info['quantidade']}"
                    )
                ...
                print(
                    f"Aviso: O medicamento '{nome_item.capitalize()}' não foi encontrado no estoque e será ignorado."
                )
        ...
        for item in venda.get_comprovante_itens():
            print(
                f"{item['nome']:<30}{item['qtd']:<5}{item['preco_unit']:>7.2f}{item['subtotal']:>8.2f}"
            )
        print("-" * 50)
        ...
        try:
            with open("database/vendas.txt", "a", encoding="utf-8") as arquivo:
