            ...
            match op:
                case "1":
                    ...
                    try:
                        ...
                        prescricao_input = (
                            input("Requer prescrição? (s/n): ").strip().lower()
                        )
                        ...
                        tarjaPreta_input = (
                            input("É tarja preta? (s/n): ").strip().lower()
                        )
                        ...
                        with open("database/medicamentos.txt", "a", encoding="utf-8") as arquivo:
                            arquivo.write(f"nome:{medicamento.get_nome()}\n")
                            arquivo.write(
                                f"fabricante:{medicamento.get_fabricante()}\n"
                            )
                            arquivo.write(f"dosagem:{medicamento.get_dosagem()}\n")
                            arquivo.write(f"preco:{medicamento.get_preco()}\n")
                            arquivo.write(
                                f"quantidade:{medicamento.get_quantidade()}\n"
                            )
                            arquivo.write(f"tipo:{medicamento.get_tipo()}\n")
                            arquivo.write(
                                f"prescricao:{medicamento.get_prescricao()}\n"
                            )
                            arquivo.write(
                                f"tarjaPreta:{medicamento.get_tarjaPreta()}\n\n"
                            )
                        ...
                        print(
                            "Erro: O arquivo 'database/medicamentos.txt' não foi encontrado."
                        )
                case "2":
                    limpar_terminal()
                    print("\nRemover Medicamento selecionado.")
                    nome_deletar = input(
                        "\nDigite o nome do medicamento a ser deletado: "
                    ).strip()
                    ...
                    try:
                        ...
                        while i < len(linhas):
                            ...
                            if (
                                linha_atual.startswith("nome:")
                                and linha_atual.split(":", 1)[1].strip().lower()
                                == nome_deletar.lower()
                            ):
                        ...
                        if medicamento_encontrado:
                            with open("database/medicamentos.txt", "w", encoding="utf-8") as arquivo:
                ...
                case "3":
                    ...
                    nome_atualizar = input(
                        "\nDigite o nome do medicamento que deseja atualizar: "
                    ).strip()
                        ...
                        while i < len(linhas):
                            ...
                            if (
                                ...
                                try:
                                    novo_preco = float(
                                        input(
                                            f"Digite o novo preço do medicamento (preço atual {linhas[i+3].split(':', 1)[1].strip()}): "
                                        )
                                    )
                                    nova_quantidade = int(
                                        input(
                                            f"Digite a nova quantidade em estoque (estoque atual {linhas[i+4].split(':', 1)[1].strip()}): "
                                        )
                                    )
                                except ValueError:
                                    print(
                                ...
                                novas_linhas.append(
                                    f"nome:{medicamento_atualizado.get_nome()}\n"
                                )
                                novas_linhas.append(
                                    f"fabricante:{medicamento_atualizado.get_fabricante()}\n"
                                )
                                novas_linhas.append(
                                    f"dosagem:{medicamento_atualizado.get_dosagem()}\n"
                                )
                                novas_linhas.append(
                                    f"preco:{medicamento_atualizado.get_preco()}\n"
                                )
                                novas_linhas.append(
                                    f"quantidade:{medicamento_atualizado.get_quantidade()}\n"
                                )
                                novas_linhas.append(
                                    f"tipo:{medicamento_atualizado.get_tipo()}\n"
                                )
                                novas_linhas.append(
                                    f"prescricao:{medicamento_atualizado.get_prescricao()}\n"
                                )
                                novas_linhas.append(
                                    f"tarjaPreta:{medicamento_atualizado.get_tarjaPreta()}\n"
                                )
                                novas_linhas.append("\n")
                ...
                case "6":
                    ...
                    nome_pesquisar = (
                        input("\nDigite o nome do medicamento a ser pesquisado: ")
                        .strip()
                        .lower()
                    )
                    medicamento_encontrado = False
